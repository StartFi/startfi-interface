...
export const useStartFiMarketplace = (withSignerIfPossible?: boolean): Contract | null => {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId && STARTFI_MARKET_PLACE_NETWORK[3].address, STARTFI_MARKET_PLACE_ABI, withSignerIfPossible)
}
export const useStartFiRoyality = (withSignerIfPossible?: boolean): Contract | null => {
  const { chainId } = useActiveWeb3React()
  return useContract(chainId && STARTFI_RoyaltyNFT[3].address, STARTFI_RoyaltyNFTI, withSignerIfPossible)
}
